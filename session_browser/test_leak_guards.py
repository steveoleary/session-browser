...
def guard_repo(tmp_path: Path) -> GuardRepo:
    ...
    fake_bd = fake_bin / "trk"
    fake_bd.write_text(
        """#!/bin/sh
if [ "$1 $2 $3" = "hooks install --tracker" ]; then
  git config core.hooksPath "$(pwd)/.tracker/hooks"
  mkdir -p .tracker/hooks
  for name in pre-commit post-merge pre-push post-checkout prepare-commit-msg; do
    hook=".tracker/hooks/$name"
    if [ ! -f "$hook" ]; then
      printf '%s\n' '#!/bin/sh' > "$hook"
      printf '%s\n' '# --- BEGIN TRACKER INTEGRATION test ---' >> "$hook"
      printf '%s\n' ':' >> "$hook"
      printf '%s\n' '# --- END TRACKER INTEGRATION test ---' >> "$hook"
    fi
    chmod +x "$hook"
  done
  exit 0
...
def test_installer_combines_tracker_and_project_hooks_across_regeneration(
    guard_repo: GuardRepo,
) -> None:
    (guard_repo.root / ".tracker").mkdir()
    guard_repo.configure_marker()
    ...
    assert guard_repo.git("config", "--get", "core.hooksPath").stdout.strip() == str(
        guard_repo.root / ".tracker/hooks"
    )
    pre_commit = guard_repo.root / ".tracker/hooks/pre-commit"
    commit_msg = guard_repo.root / ".tracker/hooks/commit-msg"
    first_pre_commit = pre_commit.read_text()
    assert "BEGIN TRACKER INTEGRATION" in first_pre_commit
    assert "BEGIN LEAK-GUARD" in first_pre_commit
    assert first_pre_commit.index("END TRACKER INTEGRATION") < first_pre_commit.index(
        "BEGIN LEAK-GUARD"
    )
    assert "BEGIN LEAK-GUARD-MSG" in commit_msg.read_text()

    tracker_start = first_pre_commit.index("# --- BEGIN TRACKER INTEGRATION")
    tracker_end = first_pre_commit.index("# --- END TRACKER INTEGRATION", tracker_start)
    tracker_end = first_pre_commit.index("\n", tracker_end) + 1
    regenerated = (
        first_pre_commit[:tracker_start]
        + "# --- BEGIN TRACKER INTEGRATION regenerated ---\n"
        + ": # regenerated by a newer tracker release\n"
        + "# --- END TRACKER INTEGRATION regenerated ---\n"
        + first_pre_commit[tracker_end:]
    )
    ...
    assert checked.returncode == 0, checked.stderr
    assert "hook path   .tracker/hooks active" in checked.stdout
    assert "tracker       all managed hooks installed" in checked.stdout
    refreshed = pre_commit.read_text()
    assert "regenerated by a newer tracker release" in refreshed
    ...
    staged = guard_repo.script(".tracker/hooks/pre-commit")
    message = guard_repo.script(".tracker/hooks/commit-msg", str(message_file))
