...
def test_installer_follows_core_hookspath_and_preserves_other_sections(
    guard_repo: GuardRepo,
) -> None:
    """The guards must land in the directory git really runs hooks from.

    Resolving that by looking for a marker directory on disk, rather than by
    asking git, once put the guards in one directory while git ran another --
    and made --check report a hook as missing while the hook git actually ran
    carried it. core.hooksPath is the only answer that binds.
    """
    elsewhere = guard_repo.root / "hooks-elsewhere"
    elsewhere.mkdir()
    guard_repo.git("config", "core.hooksPath", str(elsewhere))
    guard_repo.configure_marker()

    # A section owned by some other tool, already in the hook the installer is
    # about to write to. It must survive, markers and body alike.
    other_begin = "# --- BEGIN OTHER TOOL ---"
    other_end = "# --- END OTHER TOOL ---"
    pre_commit = elsewhere / "pre-commit"
    ...
    # Written to core.hooksPath, not to .git/hooks.
    assert not (guard_repo.root / ".git/hooks/commit-msg").exists()
    assert "BEGIN LEAK-GUARD-MSG" in (elsewhere / "commit-msg").read_text()

    first_pre_commit = pre_commit.read_text()
    assert other_begin in first_pre_commit
    assert "BEGIN LEAK-GUARD" in first_pre_commit
    assert first_pre_commit.index(other_end) < first_pre_commit.index(
        "BEGIN LEAK-GUARD"
    )

    # The other tool regenerates its own section; ours must not be duplicated or
    # lost when the installer runs again over the rewritten file.
    other_start = first_pre_commit.index(other_begin)
    other_stop = first_pre_commit.index("\n", first_pre_commit.index(other_end)) + 1
    pre_commit.write_text(
        first_pre_commit[:other_start]
        + f"{other_begin}\n: # regenerated by the other tool\n{other_end}\n"
        + first_pre_commit[other_stop:]
    )
    ...
    # --check names the directory it inspected, so a disagreement is settleable.
    assert "hook path   hooks-elsewhere" in checked.stdout
    assert "other sections present and preserved" in checked.stdout

    refreshed = pre_commit.read_text()
    assert "regenerated by the other tool" in refreshed
    assert refreshed.count("BEGIN LEAK-GUARD (") == 1

    # Both halves still fire, from the directory git is using.
    guard_repo.write("staged.txt", f"contains {MARKER}\n")
    guard_repo.git("add", "staged.txt")
    message_file = guard_repo.write(".git/COMMIT_EDITMSG", f"mentions {MARKER}\n")

    staged = guard_repo.script("hooks-elsewhere/pre-commit")
    message = guard_repo.script("hooks-elsewhere/commit-msg", str(message_file))
