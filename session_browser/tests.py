...
class TestHerdrInBandResize:
    """Inside herdr the TUI must never turn on in-band resize reports.

    herdr answers Textual's 2048 query as supported and sends resize reports,
    but sends mouse positions in cells; Textual scales them as pixels from the
    first report onward. Every click then lands near the top-left corner.
    """

    # What herdr sent a full-size 162x52 pane that had never been resized.
    REPORT = "\x1b[48;52;162;1768;2592t"
    CLICK_ROW_10_COL_20 = "\x1b[<0;21;11M"

    def test_textual_scales_cell_clicks_after_a_resize_report(self):
        """The upstream behaviour the driver override exists for.

        If this starts failing, Textual has stopped scaling unconfirmed
        coordinates and `_without_in_band_resize` may be removable.
        """
        from textual._xterm_parser import XTermParser

        parser = XTermParser(False)
        (before,) = parser.feed(self.CLICK_ROW_10_COL_20)
        list(parser.feed(self.REPORT))
        (after,) = parser.feed(self.CLICK_ROW_10_COL_20)

        assert (before.x, before.y) == (20, 10)
        assert (after.x, after.y) == (1, 0)

    @staticmethod
    def _driver_class(monkeypatch, herdr_env: str | None):
        from session_browser.app import SessionBrowser

        if herdr_env is None:
            monkeypatch.delenv("HERDR_ENV", raising=False)
        else:
            monkeypatch.setenv("HERDR_ENV", herdr_env)
        return SessionBrowser().get_driver_class()

    def test_outside_herdr_the_stock_driver_is_untouched(self, monkeypatch):
        from textual.app import App

        stock = App().get_driver_class()
        assert self._driver_class(monkeypatch, None) is stock

    def test_inside_herdr_the_2048_answer_never_reaches_the_driver(self, monkeypatch):
        """Swallowing the answer is what keeps 2048 and 1016 switched off.

        The stock driver answers `InBandWindowResize` by writing `?2048h` and
        `?1016h`; a message it never sees enables nothing, so herdr is never
        asked for a report. Anything else still goes through.
        """
        from textual import events
        from textual.app import App
        ...
        stock = App().get_driver_class()
        driver_class = self._driver_class(monkeypatch, "1")
        assert driver_class is not stock
        assert issubclass(driver_class, stock)

        seen = []
        monkeypatch.setattr(
            stock, "process_message", lambda self, message: seen.append(message)
        )
        # Bypass __init__: the real one installs signal handlers on this process.
        driver = object.__new__(driver_class)
        key = events.Key("x", "x")

        driver.process_message(InBandWindowResize(True, False))
        driver.process_message(InBandWindowResize(True, True))
        driver.process_message(key)
