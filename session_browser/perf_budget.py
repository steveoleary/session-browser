...
def _committed_json(path: Path) -> dict | None:
    """*path* as HEAD has it, or None when there is no committed version.

    None covers every "there is nothing to compare against" case identically:
    no git, not a work tree, the file untracked or not yet committed, a
    checkout with no HEAD, or a committed version that will not parse. Each of
    those means the caller should fall back to the working copy, and none of
    them is an error worth stopping a bless for.
    """
    try:
        root = subprocess.run(
        ...
        # ValueError covers relative_to on a path outside the work tree.
        return None


def committed_baseline() -> tuple[dict[str, dict[str, int]], dict[str, int]] | None:
    """The budgets as the last commit has them, for the movement record.

    A bless compares against this rather than against the working copy,
    because the ``_blessed`` block's whole job is to describe the diff a
    reviewer will read. Compared against the working copy it described the
    last *bless* instead, so blessing twice -- which is what happens when
    anything in the generated file is adjusted and re-recorded -- replaced a
    movement record with an empty one, and the commit then asserted that no
    counter moved. No hand-edit, no flag, no warning.
    """
    data = _committed_json(BUDGETS_PATH)
    if data is None:
        return None
    return data.get("workloads", {}), data.get("loop_opcodes", {})
...
def save_budgets(
    ...
    # Two different baselines, deliberately. The MERGE keeps the working copy,
    # so blessing on a machine without ripgrep cannot delete the budgets for
    # the workloads it could not run. The RECORD is against the last commit,
    # so it describes the diff a reviewer will read rather than the last time
    # anyone ran bless.
    committed = committed_baseline()
    base, base_loops = (
        committed if committed is not None else (previous, existing_loops)
    )
    # Computed before the merge below overwrites the numbers it compares against.
    blessed = blessed_delta(base, measured, base_loops, loops)
        ...
        json.dumps(
            {
                "_comment": (
                    ...
                    "timings. _blessed records what this file's committed version "
                    "says versus what is here now; it is derived, not written by "
                    "hand, so it is the diff's own account of whether a regression "
                    "was accepted -- and it survives being re-blessed."
                ),
                "_blessed": blessed,
                "guards": guards,
