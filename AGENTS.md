...
`trk hooks install --tracker`. That makes `.tracker/hooks` the active hook directory
through `core.hooksPath`; the project hooks are added outside Tracker' managed
markers, so either installer can regenerate its own section safely. A public
clone with no `.tracker` workspace keeps Git's normal `.git/hooks` directory and
does not need Tracker.
...
preserved. `--check` reports both hooks separately; in a Tracker workspace it also
verifies that `.tracker/hooks` is active and all five Tracker hooks are installed.
